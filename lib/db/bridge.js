...
function toSqlColumn(column, baseAlias = 't', options = {}) {
  const { allowQualified = true } = options;
  const text = String(column);
  if (text.includes('.')) {
    if (!allowQualified) {
      throw new Error(`El filtro ${text} no esta permitido en esta operacion.`);
    }
...
function buildWhereClause(state, values, options = {}) {
  const { baseAlias = 't', allowQualified = true } = options;
  const clauses = [];
  for (const filter of state.filters || []) {
    const column = toSqlColumn(filter.column, baseAlias, { allowQualified });
    ...
    if (filter.type === 'in') {
      const arrayValue = Array.isArray(filter.value) ? filter.value.map((value) => normalizeValue(value)) : [];
      if (!arrayValue.length) {
        clauses.push('1=0');
        continue;
      }
      const index = values.push(arrayValue);
...
function buildWriteReturningProjection(table, selectColumns) {
  const parts = splitTopLevel(selectColumns || '*').map(parseSelectPart);
  const selectItems = [];
  ...
  for (const part of parts) {
    ...
    if (part.kind === 'join') {
      throw new Error('RETURNING en escrituras solo admite columnas de la tabla modificada (sin joins ni alias).');
    }

    const column = String(part.field || '').trim();
    if (!column || column === '*') {
      selectItems.push('*');
      continue;
    }
    ...
    if (column.includes('.')) {
      throw new Error(`RETURNING invalido: ${column}. Usa solo columnas directas de ${table}.`);
    }

    selectItems.push(quoteIdent(column));
  }
...
async function executeSelect(client, state) {
  ...
  sql += buildWhereClause(state, values, { baseAlias: 't', allowQualified: true });
  ...
  if (state.count === 'exact' && state.head) {
    const countSql = `select count(*)::int as count${sql.slice(sql.indexOf(' from public.'))}`;
...
async function executeInsert(client, state) {
  ...
  if (state.selectColumns) {
    const projection = buildWriteReturningProjection(state.table, state.selectColumns);
    sql += ` returning ${projection.join(', ')}`;
  }
...
async function executeUpdate(client, state) {
  ...
  sql += buildWhereClause(state, params, { baseAlias: null, allowQualified: false });
  if (state.selectColumns) {
    const projection = buildWriteReturningProjection(state.table, state.selectColumns);
    sql += ` returning ${projection.join(', ')}`;
  }
...
export class PgLikeBuilder {
  ...
  in(column, values) { this.state.filters.push({ type: 'in', column, value: values }); return this; }
